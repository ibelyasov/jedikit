...
Проверено: **2026-08-09**. Исследования собраны до проектирования и реализации. Финальный бренд — **JediKit**; канонический package/repo slug — `jedikit`, первый skill — `jedikit-tasks`. Текущий GitHub-репозиторий ещё не переименован.
...
- Только явный capture («запиши/сохрани, чтобы не забыть») сохраняет исходную фразу во Inbox. Обычный ответ или замеченная агентом мысль не записываются. Во время поштучного triage после подтверждения фраза заменяется канонической формулировкой; история и служебный `raw_text` не сохраняются.
- Агент различает задачу, проект, идею, справку и встречу. В v1 он управляет задачами и минимальным проектным контуром; для остальных типов объясняет различие и предлагает пользователю выбрать место или создать prep/follow-up task.
...
- Hosted MCP не предоставляет подтверждённого permanent delete или настоящего batch. v1 их не обещает и не имитирует транзакционность; согласованная группа выполняется последовательными наблюдаемыми операциями.
...
- Автоматически сохраняются только минимальные настройки и техническое состояние: IDs/режимы корневых областей и timestamps завершённых обзоров.
...
Grilling и naming завершены. Источник истины для продуктовых решений — [product-decisions.md](product-decisions.md); отложенные направления — [BACKLOG.md](../BACKLOG.md). Следующий продуктовый шаг — implementation plan для `jedikit-tasks`.
