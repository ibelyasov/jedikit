...
> **Product overlay 2026-08-09:** заголовок и примеры ниже сохраняют временное кодовое имя исследования. Релизный plugin — `jedikit` с одним skill `jedikit-tasks`, без router. Возможности Cloud/Desktop/`/loop` исследованы как факты платформы, но если подходящий native scheduler недоступен, product skill только объясняет ограничение и не предлагает GitHub Actions, cron или launchd fallback.
...
4. `/loop` и встроенный CronCreate — временные задания текущей сессии: нужен запущенный Claude Code, повторение истекает через 7 дней. Долговечные native surfaces — Cloud Routine (`/schedule`) и Desktop scheduled task; если ни одна не подходит, product skill только сообщает об ограничении ([scheduled tasks](https://code.claude.com/docs/en/scheduled-tasks), [web scheduled tasks](https://code.claude.com/docs/en/web-scheduled-tasks)).
...
| Репозиторий, PR или отчёт без локального диска | Cloud Routine через `/schedule`. |
| Локальные данные, доступные разрешённому skill/MCP | Desktop Local scheduled task; компьютер должен быть включён. |
| Native scheduler не подходит или недоступен | Объяснить ограничение; не предлагать внешний fallback от имени product skill. |
