...
- **Заменить в skill:** спросить только уровень ресурса, разделить deadline и выбранный start, сформировать focus list и явно сказать, что календарь/доступное время не проверены. Любой перенос/изменение даты — только как preview с подтверждением; убрать фиксированные часы, 6h и квоты 3–5/2–3/2–3.
...
- **Заменить в skill:** временно показать исходный Inbox capture, спросить «это действие, проект, идея, справка или встреча?», предложить один физический шаг и после подтверждения заменить исходный текст финальной формулировкой без истории `raw_text`; дату не проставлять по умолчанию. Архив/отмена — отдельные ветви с подтверждением, а не три фиксированных действия.
...
- **Заменить в skill:** согласованный weekly проход Inbox и всех активных реальных проектов, раздельные факты «отменено» и «изменено», oldest inbox и один следующий шаг на проект. Календарь не читать и явно назвать это ограничение. Любые cleanup или переносы — только предложить и дождаться решения.
...
| --- | --- | --- | --- |
| Существуют prompts | 4 имена и точные metadata возвращены `prompts/list` | Список может измениться; `listChanged:false`, push не обещан | Сохранить только как research evidence; core v1 не показывает и не вызывает их |
